...
def main():
    clear()
    setTitle(f"333 Menu v{THIS_VERSION}")
    astraahometitle()
...
\n          {y}[{w}02{y}]{w} Rat (Soon)             {y}[{w}07{y}]{w} Image                   {y}[{w}10{y}]{w} Account Disabler         {y}[{w}16{y}]{w} Clear DM
...
\n          {y}[{w}05{y}]{w} VideoCrash Maker (Patched)                          {y}[{w}13{y}]{w} Token Informations       {y}[{w}19{y}]{w} Mass DM
...
if __name__ == "__main__":
    ...
    os.system("""if not exist "util/chromedriver.exe" curl -#fkLo "util/chromedriver.exe" "https://github.com/MattSWTZ/chromedriver/blob/main/chromedriver.exe?raw=true" """)
    if os.path.basename(sys.argv[0]).endswith("exe"):
        search_for_updates()
        if not os.path.exists(getTempDir()+"\\333_proxies"):
            proxy_scrape()
        clear()
        main()
    try:
        ...
        input(f"{y}[{Fore.RED}#{y}]{w} Sorry but, your python version ({sys.version_info[0]}.{sys.version_info[1]}.{sys.version_info[2]}) is not compatible with 333, please download python 3.9 or higher.")
        sys.exit()
    else:
        search_for_updates()
        if not os.path.exists(getTempDir()+"\\333_proxies"):
